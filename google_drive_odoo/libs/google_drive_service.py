...
class GoogleDriveApiClient():
    ...
    def _upload_file(self, folder, file_name, mimetype, path, file_size):
        """
        ...
        with open(path, "rb") as f:
            upload_session = self._generate_upload_session(
                folder=folder, file_name=file_name, mimetype=mimetype, file_size=file_size,
            )
            uploaded_bytes = 0
            while uploaded_bytes < file_size:
                ...
                res = self._request_gd(
                    method="POST", url=upload_session, data=chunk, headers=headers, content_type=mimetype,
                    timeout=self.TIMEOUT, full_url=True,
                )
                uploaded_bytes += chunk_length
        return res and res.json() or False
    ...
    def _request_gd(self, method, url, params={}, data={}, headers={}, content_type="application/json",
        ...
        if self.team_drive_id:
            params.update({"supportsTeamDrives": True, "driveId": self.team_drive_id})
        if method.upper() in ("GET", "DELETE"):
            ...
            raise Exception("The method {} is not supported. Not in [GET, POST, PUT, PATCH or DELETE]!".format(method))
        if res.status_code in (200, 201, 202, 308):
            return res
